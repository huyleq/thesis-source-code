/*
 *  SortSEP < infile.H > outfile.H keys="[+-]key1,[+-]key2,...,[+-]keyn" verbose=n synch=n
 */
...
sep_add_doc_line("    SortSEP - sort by sep3d header keys");\
sep_add_doc_line("");\
sep_add_doc_line("SYNOPSIS");\
sep_add_doc_line("    SortSEP < in.H > out.H keys=\"[+-]key1,[+-]key2,...,[+-]keyN\" synch=n verbose=n");\
sep_add_doc_line("");\
...
#define FLOATKEY 2
  ...
  if(0 == fetch("d1","f",&d1)) { seperr("Missing d1!\n"); }
  if(0 == fetch("o1","f",&o1)) { seperr("Missing o1!\n"); }
  if(0 == fetch("n2","d",&n2)) { seperr("Missing n2!\n"); }
  keybuffer[0] = '\0';
  ...
  if(verbose) fprintf(stderr,"number of keys is %d\n",nkeys);
  ...
  if(verbose) {
     for(ikey=0; ikey<nkeys; ++ikey) {
       fprintf(stderr,"key%d is \"%s\" sort direction %c\n",ikey+1,keylist[ikey],sortdir[ikey]);
     }
  }
  ...
  /* If we're reordering headers and data, so tell output to ignore */
  ...
  inhdr = (int *) calloc(nbytesHdr+sizeof(int)/*room for new drn*/,sizeof(char));
  if(inhdr == ((int *) NULL)) {
      seperr("Unable to allocate memory!\n");
  }
  ...
  if(cpid == 0) { /* child launches Unix sort */
     ...
     for(ikey=0; ikey<nkeys; ++ikey) {
       ...
       sprintf(tempbuf,"%d,%dn",ikey+2,ikey+2);
     ...
     istat = execvp("/usr/bin/sort",arglist);
