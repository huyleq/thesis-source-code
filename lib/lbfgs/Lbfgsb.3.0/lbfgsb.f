...
c
c     n is an integer*8 variable.
c       On entry n is the dimension of the problem.
c       On exit n is unchanged.
c
c     m is an integer*8 variable.
...
c
c     nbd is an integer*8 array of dimension n.
...
c
c     iwa is an integer*8 working array of length 3nmax.
c
...
c
c     iprint is an integer*8 variable that must be set by the user.
...
c                                     bounds;
c
c     isave is an integer*8 working array of dimension 44.
c       On exit with 'task' = NEW_X, the following information is 
c                                                             available:
...
c======================= The end of setulb =============================
     ...
     +                  iprint, csave, lsave, isave, dsave)
      ...
      character*60     task, csave
      logical          lsave(4)
      integer*8          n, m, iprint, nbd(n), index(n),
...
c
c     index is an integer*8 working array of dimension n.
c       In subroutine freev, index is used to store the free and fixed
c          variables at the Generalized Cauchy Point (GCP).
c
c     iwhere is an integer*8 working array of dimension n used to record
...
c
c     indx2 is an integer*8 working array of dimension n.
...
c     lsave is a logical working array of dimension 4.
c
c     isave is an integer*8 working array of dimension 23.
c
c     dsave is a real working array of dimension 29.
c
c
...
c     ************
      ...
      character*3      word
      integer*8          i,k,nintol,itfile,iback,nskip,
     +                 head,col,iter,itail,iupdat,
     +                 nseg,nfgv,info,ifun,
     +                 iword,nfree,nact,ileave,nenter
      real theta,fold,sdot,dr,rr,tol,
...
c                                          restore the previous iterate.
               call scopy(n,t,1,x,1)
               call scopy(n,r,1,g,1)
               f = fold
            endif
            goto 999
         endif
...
c======================= The end of mainlb =============================
     ...
     +                  prjctd, cnstnd, boxed)
      ...
      integer*8          n, iprint, nbd(n), iwhere(n)
      real x(n), l(n), u(n)
...
c
c     iwhere is an integer*8 array of dimension n.
...
c
c     col is an integer*8 variable.
...
c     Solve the triangular system
      call strsl(wt,m,col,p(col+1),11,info)
      if (info .ne. 0) return
...
c       solve J^Tp2=p2. 
      call strsl(wt,m,col,p(col+1),01,info)
      if (info .ne. 0) return
...
c======================== The end of bmv ===============================
     ...
     +                  v, nseg, iprint, sbgnrm, info, epsmch)
      implicit none
      integer*8          n, m, head, col, nseg, iprint, info, 
...
c
c     iorder is an integer*8 working array of dimension n.
...
c       On exit col is unchanged.
c
c     head is an integer*8 variable.
...
c
c     nseg is an integer*8 variable.
...
c     ************
      ...
      integer*8          i,j,col2,nfree,nbreak,pointr,
     +                 ibp,nleft,ibkmin,iter
      real f1,f2,dt,dtm,tsum,dibp,zibp,dibp2,bkmin,
     +                 tu,tl,wmc,wmp,wmw,sdot,tj,tj0,neggi,sbgnrm,
     +                 f2_org
      real one,zero
      parameter        (one=1.0d0,zero=0.0d0)
...
c                   complete the initialization of p for theta not= one.
         call sscal(col,theta,p(col+1),1)
      endif
...
c           compute (wbp)Mc, (wbp)Mp, and (wbp)M(wbp)'.
         ...
         if (info .ne. 0) return
         wmc = sdot(col2,c,1,v,1)
         wmp = sdot(col2,p,1,v,1) 
         wmw = sdot(col2,wbp,1,v,1)
 
c           update p = p - dibp*wbp. 
         call saxpy(col2,-dibp,wbp,1,p,1)
...
c       which will be used in computing r = Z'(B(x^c - x) + g).
      ...
      if (col .gt. 0) call saxpy(col2,dtm,p,1,c,1)
      if (iprint .gt. 100) write (6,1010) (xcp(i),i = 1,n)
...
c====================== The end of cauchy ==============================
     ...
     +                 theta, col, head, nfree, cnstnd, info)
      ...
      logical          cnstnd
      integer*8          n, m, col, head, nfree, info, index(n)
      real theta, 
...
c======================= The end of errclb =============================
     ...
     +                 head, info)
      ...
      integer*8          n, nsub, m, col, head, nenter, ileave, iupdat,
     +                 info, ind(n), indx2(n)
...
c
c     iupdat is an integer*8 variable.
...
c     ************
      ...
      integer*8          m2,ipntr,jpntr,iy,is,jy,js,is1,js1,k1,i,k,
     +                 col2,pbegin,pend,dbegin,dend,upcl
      real sdot,temp1,temp2,temp3,temp4
      real one,zero
      parameter        (one=1.0d0,zero=0.0d0)
...
c                                 shift old part of WN1.
            do 10 jy = 1, m - 1
               js = m + jy
               call scopy(m-jy,wn1(jy+1,jy+1),1,wn1(jy,jy),1)
               call scopy(m-jy,wn1(js+1,js+1),1,wn1(js,js),1)
               call scopy(m-1,wn1(m+2,jy+1),1,wn1(m+1,jy),1)
  10        continue
         endif
...
c        then form L^-1(-L_a'+R_z') in the (1,2) block.
      col2 = 2*col
      do 71 js = col+1 ,col2
         call strsl(wn,m2,col,wn(1,js),11,info)
  71  continue
...
c        upper triangle of (2,2) block of wn.
      ...
      do 72 is = col+1, col2
         do 74 js = is, col2
               wn(is,js) = wn(is,js) + sdot(col,wn(1,is),1,wn(1,js),1)
  74        continue
  72     continue

c     Cholesky factorization of (2,2) block of wn.

      call spofa(wn(col+1,col+1),m2,col,info)
      if (info .ne. 0) then
         info = -2
         return
      endif
...
c======================= The end of formt ==============================
     ...
     +                 iwhere, wrk, updatd, cnstnd, iprint, iter)
      ...
      integer*8 n, nfree, nenter, ileave, iprint, iter, 
...
c
c     index is an integer*8 array of dimension n
...
c
c     iorder is an integer*8 array of dimension n.
...
c
c     iheap is an integer*8 variable specifying the task.
...
c====================== The end of hpsolb ==============================
     ...
     +                  isave, dsave)
      ...
      integer*8          n, iter, ifun, iback, nfgv, info,
     +                 nbd(n), isave(2)
...
c     Determine the maximum step length.
  ...
  43        continue
      ...
      call scopy(n,x,1,t,1)
      call scopy(n,g,1,r,1)
      fold = f
      ifun = 0
      iback = 0
      csave = 'START'
...
c======================= The end of lnsrlb =============================
     ...
     +                  iupdat, col, head, theta, rr, dr, stp, dtd)
      ...
      integer*8          n, m, itail, iupdat, col, head
...
c     Update matrices WS and WY.
      ...
      call scopy(n,d,1,ws(1,itail),1)
      call scopy(n,r,1,wy(1,itail),1)
...
c                              move old information
         do 50 j = 1, col - 1
            call scopy(j,ss(2,j+1),1,ss(1,j),1)
            call scopy(col-j,sy(j+1,j+1),1,sy(j,j),1)
  50     continue
      endif
...
c                                             and the last column of SS:
      pointr = head
      do 51 j = 1, col - 1
         sy(col,j) = sdot(n,d,1,wy(1,pointr),1)
         ss(j,col) = sdot(n,ws(1,pointr),1,d,1)
         pointr = mod(pointr,m) + 1
  51  continue
      if (stp .eq. one) then
         ss(col,col) = dtd
...
c======================= The end of prn1lb =============================
     ...
     +                  sbgnrm, nseg, word, iword, iback, stp, xstep)
      ...
      character*3      word
      integer*8          n, iprint, itfile, iter, nfgv, nact, nseg,
     +                 iword, iback
...
c======================= The end of prn2lb =============================
     ...
     +                  cachyt, sbtime, lnscht)
      ...
      character*60     task
      character*3      word
      integer*8          n, iprint, info, itfile, iter, nfgv, nintol,
...
c======================= The end of projgr =============================
     ...
     +                   col, head, iword, wv, wn, iprint, info )
      implicit none
      integer*8          n, m, nsub, col, head, iword, iprint, info, 
     +                 ind(nsub), nbd(n)
      real theta, 
...
c       On exit u is unchanged.
c
c     nbd is a integer*8 array of dimension n.
c       On entry nbd represents the type of bounds imposed on the
c         variables, and must be specified as follows:
c         nbd(i)=0 if x(i) is unbounded,
c                1 if x(i) has only a lower bound,
...
c       On exit they are unchanged.
c
c     iword is an integer*8 variable.
...
c     ************
      ...
      integer*8          pointr,m2,col2,ibd,jy,js,i,j,k
...
c     Compute wv:=K^(-1)wv.
      ...
      m2 = 2*m
      col2 = 2*col
      call strsl(wn,m2,col2,wv,11,info)
      if (info .ne. 0) return
      do 25 i = 1, col
         wv(i) = -wv(i)
  25     continue
      call strsl(wn,m2,col2,wv,01,info)
      if (info .ne. 0) return
...
c
c       isave is an integer*8 work array of dimension 2.
c         
