...
void newton(size_t n,float *x,float &f,float *g,float *diag,float *w,int &iflag,int *isave,float *dsave,Operator *H,int niter_max){
    ...
    /*
     ...
     This subroutine solves the unconstrained minimization problem
      ...
      The calling statement is 
          ...
          CALL NEWTON(N,X,F,G,DIAG,W,IFLAG)
     ...
     W       is a REAL array of length 2*N+1 used to store the search direction, 
             the previous gradient and it norm.
    ...
    */
    if(iflag==0){
        ...
        /*
         ...
         THE PREVIOUS GRADIENT AND ITS NORM.
         */
        int niter=indefCG(H,w,g,niter_max);
        fprintf(stderr,"inner loop terminated after %d iterations\n",niter);
        
        nfun+=2*niter;
        if(niter==1){
            float wnorm=sqrt(sdot(n,w,w));
            stp1=1.f/wnorm;
        }
        else stp1=1.f;
