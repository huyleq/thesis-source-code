    ...
    /*
     ...
     This subroutine solves the unconstrained minimization problem
      ...
      The calling statement is 
          ...
          CALL NLCG(N,X,F,G,DIAG,W,IFLAG)
        ...
        */
        ...
        while(true){
            ...
            */
            ...
            nfev=0;
            float beta;
            float betaPR=(g2-sdot(n,g,w+n))/w[2*n]; //PR method
//            float betaFR=g2/w[2*n]; //FR method
//            if(betaPR<-betaFR) beta=-betaFR;
//            else if(betaPR>betaFR) beta=betaFR;
//            else beta=betaPR;
//            beta=max(betaPR,0.f);
            beta=betaPR;
            float theta=sdot(n,g,w)/w[2*n];
            #pragma omp parallel for num_threads(16)
            for(size_t i=0;i<n;i++){
                w[i]=beta*w[i]-g[i]-theta*(g[i]-w[n+i]);
                w[n+i]=g[i];
            }
            w[2*n]=g2;
        }
